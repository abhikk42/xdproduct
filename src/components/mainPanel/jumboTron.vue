<template>
  <div class="headContainer">
    ...
    <div class="actionRow">
        ...
        <button class="toggler" v-on:click="handleClick"><font-awesome-icon icon='fa-sliders' :style="{marginRight:'10px'}"/>{{togglerText}}</button>
        </div>
        <div class="rightAction">
            <div class="displayfilter">
                <p v-for="val in filterarray" :key="val.value" class="item" @click="$parent.handleRemoveFilterByJumbo(val)">{{val.value}} <font-awesome-icon icon="fa-x"  class="cross"/></p>
    ...
    <div class="bottomrow">
        <span class="clear" v-on:click="this.$parent.clearFilter" v-if="filterarray.length>0"><font-awesome-icon icon="fa-x" /> Clear All Filter</span>
    </div>
...
export default {
    ...
    mounted(){
        ...
        if(sort_by&&sort_dir)
        {
            if(sort_by=='selling_price'&&sort_dir=='asc') this.selectedValue='Price(Low to High)';
            if(sort_by=='selling_price'&&sort_dir=='desc') this.selectedValue='Price(High to Low)';
    ...
    methods:{
        handleClick()
        {
           this.togglerText= this.togglerText=='HIDE FILTER'?'SHOW FILTER':'HIDE FILTER';
